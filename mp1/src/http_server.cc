...
int main(int argc, char *argv[]) {
	...
	while(1) {  // main accept() loop
		...
		if (!fork()) { // this is the child process
			...
			int index0 = request.find_first_of("GET ");
            if (index0 != 0) {
			...
			string response = "";
            int index1 = request.find_first_of(" HTTP");
            if (index1 == request.npos) {
				response = "HTTP/1.1 400 Bad Request\r\n\r\n";
                if (send(new_fd, response.data(), response.size(), 0) == -1)
                    perror("send");
                exit(1);
            }

            string path = request.substr(1, index1-1);
			printf("server: requested file path is %s\n",path.c_str());
